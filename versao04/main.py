...
def voto_majoritario_ponderado(vizinhos_seqs, vizinhos_dist, pos_i):
    ...
    ponderada pela distância (1 / (dist + 1)).
    """
...
def rodar_validacao_ponderada(kmer_size, n_clusters, k_vizinhos):
    print(f"Iniciando VALIDAÇÃO (Otimização FINAL: CountVec + KNN Ponderado)...")
    ...
    for index, linha in df_teste_original.iterrows():
        ...
        vizinhos_dist = [distancias[i] for i in indices_vizinhos]

        for pos in indices_poked:
            base_imputada = voto_majoritario_ponderado(vizinhos_seqs, vizinhos_dist, pos)
            y_pred.append(base_imputada)

    print("\n--- Relatório de Desempenho (CountVec + KNN Ponderado) ---")
    if not y_true:
